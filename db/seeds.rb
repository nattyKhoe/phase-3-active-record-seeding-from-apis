...
#iterate over each sepll
spells.each do |spell|
    #make a req to the endpoint for the individual spell:
    response = RestClient.get "https://www.dnd5eapi.co/api/spells/#{spell}"

    #the reponse will come back as a JSON-formatted string.
    ...
    Spell.create(
        ...
        description: spell_hash["desc"][0] #spell_hash["desc"] returns an array,
    #so we need to access the first array
    )
end
